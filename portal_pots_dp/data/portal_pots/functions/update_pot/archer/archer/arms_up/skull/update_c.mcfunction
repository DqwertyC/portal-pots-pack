...
execute if score @s item_count matches 1.. run data modify entity @e[tag=portal_pot_item,tag=newest,limit=1] Item set from storage minecraft:portal_pots archer.archer.arms_up.skull.Item
...
execute unless data storage minecraft:portal_pots archer.archer.arms_up.skull.Item run scoreboard players set @s item_count 0
