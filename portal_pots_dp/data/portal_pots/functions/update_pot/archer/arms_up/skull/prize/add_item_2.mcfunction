execute unless data storage minecraft:portal_pots archer.arms_up.skull.prize.Item run data modify storage minecraft:portal_pots archer.arms_up.skull.prize.Item set from block ~ ~1 ~ Items[{Slot:2b}]
