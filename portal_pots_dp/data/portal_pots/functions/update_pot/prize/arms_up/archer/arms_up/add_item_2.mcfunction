execute unless data storage minecraft:portal_pots prize.arms_up.archer.arms_up.Item run data modify storage minecraft:portal_pots prize.arms_up.archer.arms_up.Item set from block ~ ~1 ~ Items[{Slot:2b}]
