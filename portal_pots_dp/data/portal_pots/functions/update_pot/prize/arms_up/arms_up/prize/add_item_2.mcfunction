execute unless data storage minecraft:portal_pots prize.arms_up.arms_up.prize.Item run data modify storage minecraft:portal_pots prize.arms_up.arms_up.prize.Item set from block ~ ~1 ~ Items[{Slot:2b}]
